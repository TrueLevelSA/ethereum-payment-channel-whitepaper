...
\section{Sub-channel applications} In this chapter we describe how to use sub-channels to build a partialy trusted pre-authorized payment layer. Pre-authorized payments sub-channel $\omega_p$ can then be merge into the main channel $\omega$ in a trustless way.

\subsection{Pre-authorized payments sub-channel} We define first a unique sub-channel $\omega_p$ per clients that allow the provider to take an arbitrary amount up to a limit $l$ set by the client at any moment. The rule for this sub-channel are define with the following: (i) at any moment the client risk at max $l$, (ii) $l$ can be increased instantly with cooperation between client and provider, (iii) $l$ can be decreased by $x$ instantly with cooperation if the main channel $\omega$ is increased by $x$, and (iv) $l$ can be decreased without cooperation by the client with some delay.

Sub-channel $\omega_p$ is composed of a sub-channel states $\Sigma^p$ and contract state $M^p \subset M$. Sub-channel states are noted $\sigma^p \in \Sigma^p$. We denote transitions between two sub-channel states with an increase of the limit as $\delta_{\sigma^p}$ or because of a merged into the main channel.

We define the new transition rule for contract states as
$$\mu \xrightarrow{\delta_\mu \lor \sigma \lor \sigma^p} \mu'$$

the new transition rule for main channel states as
$$\sigma \xrightarrow{\delta_\sigma \lor \sigma^p \lor \mu \rightarrow \mu'} \sigma'$$

and the new transition rule for sub-channel states as
$$\sigma^p \xrightarrow{\delta_{\sigma^p} \lor \sigma \xrightarrow{\sigma^p} \sigma'} \sigma^p'$$

We define the new transition rules as
\begin{equation*}
\begin{split}
...
\begin{equation*}
\begin{split}
    \sigma^p \xrightarrow{\delta_{\sigma^p}} \sigma^p' &: \quad \sigma^p' = \sigma^p \cup \{ \delta_{\sigma^p} \} \\
    \sigma^p \xrightarrow{\sigma \xrightarrow{\sigma^p} \sigma'} \sigma^p' &: \quad \sigma^p' = \sigma^p \setminus \{ \delta_{\sigma^p} \} \\
\end{split}
\end{equation*}

When a sub-channel state is merged into the main channel state, the sub-channel is updated and remove the delta merged. In other words the sub-channel state is no longer part of the sub-channel but part of the main channel. The delta is moved to the other layer.

\begin{equation*}
\begin{split}
    \Sigma^p&: \emptyset \xrightarrow{\delta_{\sigma^p}} \sigma_1^p \xrightarrow{\sigma_1 \xrightarrow{\sigma_1^p} \sigma_2} \sigma_2^p \\
