...
\subsubsection{Sub-channel merge} Sub-channel merges can be defined as
\begin{equation*}
\begin{split}
...
and states' set can be computed with the previous rules given the base states $\sigma^p_1$ and $\sigma_1$ as
\begin{align*}
...
We say that the main channel state contain a sub-channel state if the sub-channel state has been merged. The rule follows the subset rule
$$\sigma \subset \sigma^p \iff \exists \sigma' \mid \sigma' \xrightarrow{\sigma^p} \sigma$$

\paragraph{Channels' states pairings} We define pairings of main channel and sub-channel states. States in pairings can be eighter non-exclusive, i.e. both states can be enforced on the contract layer, or mutualy exclusive, i.e. only one of the two states can be enforced.
...
To guarentee the partialy-trusted model the pairing 1 is non-exclusive and must be valid at any moment but must invalidate the pairing 3. The inverse is also true. The pairing 2 must be mutualy exclusive, e.g. if $\sigma^p_1$ is enforced then $\sigma_2 : \sigma_2 \subset \sigma^p_1$ must be invalid in the contract layer. Otherwise deltas in $\sigma^p_1$ can be used twice and the partialy-trusted model is broken.
...
\begin{equation*}
\begin{split}
    \Sigma^p&: \emptyset \xrightarrow{\delta_{\sigma^p}} \sigma_1^p \xrightarrow{\sigma_1 \xrightarrow{\sigma_1^p} \sigma_2} \sigma_2^p \xrightarrow{\delta_{\sigma^{\prime p}}} \sigma_3^p \\
