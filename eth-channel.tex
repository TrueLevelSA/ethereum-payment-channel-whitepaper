...
Layers are linked tougether, changes in contract layer affect the channel layer and the channel layer can be dictated, or applied, on the contract layer. The full transition rule for contract states is define as
...
We can notice that each transition in contract states $M$ are also applied in channel states $\Sigma$. We denote the application of $\sigma$ on $\mu$, i.e. dictate a channel state on the contract layer, also as $\sigma(\mu) = \mu'$.

States are represented as sets of deltas $\delta \in \Delta$ and transitions are bounded to conditions. We define how to compute the resulted state.
\begin{equation*}
\begin{split}
    ...
    \sigma \xrightarrow{\mu \xrightarrow{\delta_\mu} \mu'} \sigma' &: \quad \sigma' =  \sigma \cup \mu' \lor \sigma \cup \{ \delta_\mu \} \\
...
We see that all channel states based on $\sigma_4$ will never contain $\delta_\sigma'$ becase the dictated channel state used is $\sigma_2$ and not $\sigma_3$.
\begin{align*}
